...
def test_metrics_exposes_only_upstream_error_counter(client: TestClient) -> None:
    response = client.get("/metrics")

    assert response.status_code == 200
    samples = [line for line in response.text.splitlines() if line and not line.startswith("#")]
    assert samples
    assert all(line.startswith("backend_upstream_errors_total{") for line in samples)
